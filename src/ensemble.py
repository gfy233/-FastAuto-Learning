...
Output_folder = '/data/gfy2021/gfy/KDD/stacking/stacking_result/result_sim_0.75/'

#输出的预测结果文件夹
global Output_future_folder
Output_future_folder = '/data/gfy2021/gfy/KDD/stacking/stacking_result/result_future_sim_0.75/'
...
def get_similar(data):
    ...
    watch_set = list(set(np.array(data.watch)))
    watch1_list = np.array(data[data.watch.isin([watch_set[0]])].sum_sale_14)
    watch2_list = np.array(data[data.watch.isin([watch_set[1]])].sum_sale_14)
    sum = 0
    for i in range(len(watch1_list)):
        sum += np.square(watch1_list[i]-watch2_list[i])
    distance = np.sqrt(sum)
    print("distance",distance)
    #过于相似，重新选择窗口
    if(distance < Sim_THRESHOLE):
        print("相似度不合格！")
        return False
    return True
...
def get_i_data(i,data_dict,datas):
    ...
    count =1
    #不满足相似度阈值，重新抽取
    while(not get_similar(train)):
        count+=1
      
        train_watchid = [i+5-count,i+6+count]
        print("count",count)
        try:
            print("1")
            train = datas[datas.watch.isin(train_watchid) ]
...
if __name__ == '__main__':
    ...
    for file in files: 
        ...
        for whstr in strs:
            try:
                if whstr == '0e117c1684b5ebd6093fc17b468455d1' or whstr =='0af607c2fa9f1ba55a2f31b9f4799d25':
                    pipeline(whstr)
