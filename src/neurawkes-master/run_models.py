...
def test_generalized_neural_hawkes_ctsm_and_save_time(
    ...
    for epi in range(log_dict['max_epoch']):
        ...
        for step_dev in range(data_process.max_nums[tag_split] ):
            ...
            #
            seq_type_event, seq_time_values, target_type, type_predict,prob_over_seq_over_type,target_time,time_predict = control.model_predict(
                data_process.seq_type_event_numpy,
                data_process.seq_time_values_numpy,
                data_process.seq_mask_numpy,
                time_diffs_numpy
            ...
            row = []
            for i in seq_type_event:
                row.append(i[0])
            
            
            # row = seq_type_event.reshape(1,17)
            row.append(target_type[-1][0])
            row.append(type_predict[-1][0])

            predict_y.append(type_predict[-1])
            print("predict_y: ", predict_y)
            with open("/data1/lxt/qpr/model/galanz_discount_predict/result_y/predict_y_test_PointProcess.csv",'w') as f:
                writer = csv.writer(f)
                writer.writerows(predict_y)



            row.append(abs(target_type[-1][0]- type_predict[-1][0]))
            seqs_type.append(row)
            
            print "seq_type_event: ", seq_type_event
            print "seq_time_values: ", seq_time_values
            print "target_type: ",target_type[-1][0]
            print "type_predict: ", type_predict[-1][0]
            print "prob_over_seq_over_type: ", prob_over_seq_over_type[-1][0]  #got it!!!
            all_prob_over_seq_over_type.append(prob_over_seq_over_type[-1][0])
            with open("/data1/lxt/qpr/model/galanz_discount_predict/result_proda/type_PointProcess.csv",'w') as f:
                writer = csv.writer(f)
                writer.writerows(all_prob_over_seq_over_type)
            print "type_error:", abs(target_type[-1][0]- type_predict[-1][0])
            # print "diff_type: ", diff_type
            print "target_time: ", target_time[-1][0]
            print "time_predict: ", time_predict[-1][0]
            print "time_error:", abs(target_time[-1][0]- time_predict[-1][0])
            # print "seq_mask: ", seq_mask
        print "all_prob_over_seq_over_type: ", all_prob_over_seq_over_type
        with open("./result/type_predict.csv","w") as tf:
            writer = csv.writer(tf)
            writer.writerows(seqs_type)
